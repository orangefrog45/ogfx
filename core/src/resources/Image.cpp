...
void Image2D::CreateSampler() {
    ...
    sampler_info.maxAnisotropy = VkContext::GetPhysicalDevice().vkb_device.properties.
        limits.maxSamplerAnisotropy;
    sampler_info.borderColor = vk::BorderColor::eFloatOpaqueBlack;
    sampler_info.unnormalizedCoordinates = false;
    // Coordinates are in range [0, 1], not [0, tex_pixel_extents]
    ...
    m_sampler = VkContext::GetLogicalDevice().device.createSamplerUnique(sampler_info);
}

void Image2D::BlitTo(Image2D& dst, unsigned dst_mip, unsigned src_mip,
                     vk::ImageLayout start_src_layout, vk::ImageLayout start_dst_layout,
                     vk::ImageLayout final_src_layout, vk::ImageLayout final_dst_layout,
                     vk::Filter filter, std::optional<vk::Semaphore> wait_semaphore,
    ...
    blits.srcOffsets[1] = vk::Offset3D{
        lml::max((int32_t)((float)m_spec.size.x / src_div), 1),
        lml::max((int32_t)((float)m_spec.size.y / src_div), 1), 1
    };
    ...
    blits.dstOffsets[1] = vk::Offset3D{
        lml::max((int32_t)((float)dst.m_spec.size.x / dst_div), 1),
        lml::max((int32_t)((float)dst.m_spec.size.y / dst_div), 1), 1
    };
    ...
    TransitionImageLayout(start_src_layout, vk::ImageLayout::eTransferSrcOptimal,
                          vk::AccessFlagBits::eTransferRead,
                          vk::AccessFlagBits::eTransferRead,
                          vk::PipelineStageFlagBits::eTransfer,
                          vk::PipelineStageFlagBits::eTransfer, cmd, src_mip, 1);

    dst.TransitionImageLayout(start_dst_layout, vk::ImageLayout::eTransferDstOptimal,
                              vk::AccessFlagBits::eTransferWrite,
                              vk::AccessFlagBits::eTransferWrite,
                              vk::PipelineStageFlagBits::eTransfer,
                              vk::PipelineStageFlagBits::eTransfer, cmd, dst_mip, 1);

    cmd.blitImage(m_image, vk::ImageLayout::eTransferSrcOptimal, dst.m_image,
                  vk::ImageLayout::eTransferDstOptimal, blits, filter);

    TransitionImageLayout(vk::ImageLayout::eTransferSrcOptimal, final_src_layout,
                          vk::AccessFlagBits::eTransferRead,
                          vk::AccessFlagBits::eTransferRead,
                          vk::PipelineStageFlagBits::eTransfer,
                          vk::PipelineStageFlagBits::eTransfer, cmd, src_mip, 1);

    dst.TransitionImageLayout(vk::ImageLayout::eTransferDstOptimal, final_dst_layout,
                              vk::AccessFlagBits::eTransferWrite,
                              vk::AccessFlagBits::eTransferWrite,
                              vk::PipelineStageFlagBits::eTransfer,
                              vk::PipelineStageFlagBits::eTransfer, cmd, dst_mip, 1);
    ...
    if (wait_semaphore.has_value())
        EndSingleTimeCommands(
            cmd, std::make_pair(wait_semaphore.value(),
                                vk::PipelineStageFlagBits::eTransfer));
    else
...
void Image2D::CreateImage(VmaAllocationCreateFlags flags) {
    if (m_owns_image) {
        ...
        OGFX_VK_CHECK(
            vmaCreateImage(VkContext::GetAllocator(), &im_info, &alloc_info,
...
void Image2D::CreateExternalSharedImage(VmaAllocationCreateFlags flags) {
    ...
    OGFX_VK_CHECK(vmaFindMemoryTypeIndexForImageInfo(VkContext::GetAllocator(),
        reinterpret_cast<const VkImageCreateInfo*>(&image_info), &alloc_info, &
        mem_type_idx));
...
void* Image2D::GetWin32Handle() {
    ...
    OGFX_VK_CHECK(
        vmaGetMemoryWin32Handle(VkContext::GetAllocator(), m_allocation, nullptr, &handle
        ));
    return handle;
}
...
void Image2D::GenerateMipmaps(vk::ImageLayout start_layout) {
    ...
    for (uint32_t i = 1; i < m_spec.mip_levels; i++) {
        BlitTo(*this, i, i - 1, start_layout, start_layout, start_layout, start_layout,
               vk::Filter::eLinear, std::nullopt, *cmd);
    }
    EndSingleTimeCommands(*cmd);
}
...
void Image2D::TransitionImageLayout(vk::ImageLayout old_layout,
                                    vk::ImageLayout new_layout,
                                    vk::AccessFlags src_access,
                                    vk::AccessFlags dst_access,
                                    vk::PipelineStageFlags src_stage,
                                    vk::PipelineStageFlags dst_stage,
                                    vk::CommandBuffer buf, unsigned mip_level,
    ...
    // Not transfering queue family ownership so leave at ignore
...
void Image2D::PipelineBarrier(vk::ImageLayout layout, vk::AccessFlags src_access,
                              vk::AccessFlags dst_access,
                              vk::PipelineStageFlags src_stage,
                              vk::PipelineStageFlags dst_stage, vk::CommandBuffer buf,
                              unsigned mip_level,
                              unsigned level_count) {
    TransitionImageLayout(layout, layout, src_access, dst_access, src_stage, dst_stage,
                          buf, mip_level, level_count);
}
...
void Image2D::TransitionImageLayout(vk::ImageLayout old_layout,
    ...
    } else if (old_layout == vk::ImageLayout::eUndefined && new_layout ==
    ...
    } else if (old_layout == vk::ImageLayout::eColorAttachmentOptimal && new_layout ==
    ...
    TransitionImageLayout(old_layout, new_layout, src_access, dst_access, src_stage,
                          dst_stage, buf, mip_level, level_count);
}

vk::UniqueImageView Image2D::CreateImageView(const Image2D& image,
                                             std::optional<vk::Format> fmt) {
    ...
    return VkContext::GetLogicalDevice().device.createImageViewUnique(info);
}
...
vk::RenderingAttachmentInfo Image2D::CreateAttachmentInfo(
    vk::AttachmentLoadOp load_op, vk::AttachmentStoreOp store_op, vk::ImageLayout layout,
...
vk::ImageSubresourceRange Image2D::GetFullSubresourceRange(
...
vk::DescriptorImageInfo Image2D::GetDescriptorInfo(vk::ImageLayout layout) const {
    vk::DescriptorImageInfo descriptor_image_info{};
    descriptor_image_info.sampler = *m_sampler;
    descriptor_image_info.imageView = *m_view;
    descriptor_image_info.imageLayout = layout;

    return descriptor_image_info;
}

vk::WriteDescriptorSet Image2D::GetWriteDescriptorSet(
    uint32_t binding, vk::DescriptorImageInfo* image_info) const {
