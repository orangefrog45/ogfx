...
Shader& Shader::AddStage(vk::ShaderStageFlagBits shader_stage_flags, const std::string& spv_path) {
    ...
    shader_create_info.pushConstantRangeCount = (uint32_t)m_push_constant_ranges.size();
...
void Shader::Bind(vk::CommandBuffer& cmd) {
    cmd.bindShadersEXT((uint32_t)m_shaders.size(), m_shader_stages.data(), m_shaders.data());
}

void Shader::Build() {
    m_shaders.resize(m_shader_create_infos.size());
    OGFX_VK_CHECK(VkContext::GetLogicalDevice().device.createShadersEXT(
        (uint32_t)m_shader_create_infos.size(), m_shader_create_infos.data(), nullptr, m_shaders.data()));
...
void Shader::ReflectPipelineLayout(const std::vector<std::byte>& spv) {
    ...
    for (const spirv_cross::Resource& resource : resources.push_constant_buffers) {
        ...
        push_constant_range.size = (uint32_t)compiler.get_declared_struct_size(compiler.get_type(resource.type_id));
